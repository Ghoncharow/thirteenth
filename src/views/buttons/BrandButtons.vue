        ...
        <b-card>
          ...
          <small>Usage </small><code style="text-transform:lowercase">&lt;b-button variant="facebook"&gt;&lt;span&gt;Facebook&lt;/span&gt;&lt;/b-button&gt;</code>
            ...
            <div>
              ...
              <br>
              <p>CRUD-операции в локальной базе IndexedDB:</p>
              <b-button class="mr-1" size="sm" @click="toggle1">GET</b-button>
              <b-button class="mr-1" size="sm" @click="toggle2">POST</b-button>
              <b-button class="mr-1" size="sm" @click="toggle3">PUT</b-button>
              <b-button class="mr-1" size="sm" @click="toggle4">DELETE</b-button>      
            </div>
          <hr/>
...
export default {
  ...
  methods: {
    ...
    toggle2() {
      console.log('input2:', this.text);
      var that = this.text;
      if (this.text != '') connectDB(function(db){
          var storeName = "customers";
          db.transaction([storeName], "readwrite").objectStore(storeName).put({name: that});
      });      
    },
    toggle3() {
      console.log('input3:', this.text);
      let id = parseInt(prompt('Введите нужный вам id:'));
      if (!id) return;
      var that = this.text;
      if (this.text != '') connectDB(function(db){
          var storeName = "customers";
          db.transaction([storeName], "readwrite").objectStore(storeName).put({name: that, id: id});
      });
      
    },
    toggle4() {
      console.log('input4:', this.text);
      let id = parseInt(prompt('Введите нужный вам id:'));
      if (!id) return;
      connectDB(function(db){
          var storeName = "customers";
